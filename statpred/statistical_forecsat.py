...
if __name__ == "__main__":
   ...
   s = df.loc[df['Series']==series[2],6:].dropna(axis=1, how='all').values.flatten()
   nperiod = 18
   cutpoint = -nperiod
   train = s[:cutpoint]
   test  = s[cutpoint:]
   ...
   if ch == 0:  # pmdarima
      ...
      model = pm.auto_arima(train, start_p=1, start_q=0,
                            max_p=3, max_q=3, d=1, m=12, start_P=0,
      ...
      sarima_model = SARIMAX(train, order=(2,1,1), seasonal_order=(2,0,0,12),
