...
\begin{figure}
	...
	\node (input) [block, fill=gray!10] {Input embeddings};
	\node (pe) [block, fill=gray!10, below=of input] {+ Positional Encoding};
	
	
	\node (mha) [mha, below=of pe] {Multi-Head Attention};
	\node (add1) [addnorm, below=of mha] {Add \& Norm};
	
	% Feedforward
	\node (ff) [ff, below=of add1] {Feedforward Network};
	\node (add2) [addnorm, below=of ff] {Add \& Norm};
...
~\\Line \ref{enc:attention2}: completes the computation of the attention function by implementing a weighted sum of the value vectors, $X' = A^h \cdot V^h$, thereby aggregating information from the relevant timesteps.
