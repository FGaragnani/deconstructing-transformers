...
The above results can be briefly examined. As the number of encoder and decoder blocks increases, the number of parameters (\textit{nparams}) also grows, directly influencing both the training time \textit{trtime} (expressed in seconds per epoch) and the inference time \textit{inftime}. While the training loss remains largely stable, the model’s generalization ability is reflected in the decreasing test loss values. As it can be later seen in Table \ref{tab:scal2}, there is not a direct correlation between training loss and testing loss.

Table \ref{tab:scal2} shows the result of a similar model in which both the model size and the internal embeddings' size change. In this case, the number of epochs is set to $100$: this leads to better convergence on the training set but to worse results on the training set (with respect to Table \ref{tab:scal2}). It must also be kept in mind the fact that the series at hand is relatively short; hence, beyond a certain threshold, increasing the network size - and thus the number of parameters - has only a limited effect on performances.
