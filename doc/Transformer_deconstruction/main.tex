...
\begin{figure}
    ...
    \node (input) [block, fill=gray!10] {Input embeddings \\ \scriptsize (?)};
    \node (pe) [block, fill=gray!10, below=of input] {+ Positional Encoding \\ \scriptsize (?)};
    
    % Masked MHA
    \node (mha1) [mha, below=of pe] {Masked Multi-Head Attention \\ \scriptsize (?)};
    \node (add1) [addnorm, below=of mha1] {Add \& Norm \par \scriptsize (?)};
    
    % Encoder-Decoder MHA
    \node (mha2) [mha, below=of add1] {Multi-Head Cross-Attention \\ \scriptsize (?)};
    \node (add2) [addnorm, below=of mha2] {Add \& Norm \par \scriptsize (?)};
    
    % Feedforward
    \node (ff) [ff, below=of add2] {Feedforward Network \\ \scriptsize (?)};
    \node (add3) [addnorm, below=of ff] {Add \& Norm \par \scriptsize (?)};
    
    % Output
    \node (output) [block, fill=gray!10, below=of add3] {Output Projection (Softmax)};
