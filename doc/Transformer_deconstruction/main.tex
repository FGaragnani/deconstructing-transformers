...
\textcolor{blue}{The data processing cycle consists of three main phases: encoding, decoding, and learning. A view of the whole model can be seen in Figure \ref{fig:transformer}. The following subsections provide details on each phase, the reader is referred to \cite{VSPU17} for a more exhaustive exposition.}
...
The model has also been evaluated in terms of scalability of its components (i.e., how does the number of parameters relate to training and inference time, as well as its performance).
The tests have been performed on the M3 dataset with id $2047$, against which the minimalistic model showed significantly low testing results. The outcomes of these tests are reported in Tables \ref{tab:scal1} and \ref{tab:scal2}. In Table \ref{tab:scal1}, the embedding size used was $8$ and the number of training epochs was $20$.
...
The above results can be briefly examined. First of all, as the number of encoder and decoder blocks increase, also obviously do the number of parameters (\textit{nparams}), which directly influence both the training time \textit{trtime} (expressed in seconds per epoch) and the inference time \textit{inftime} of the model. While the training loss is more or less unaltered, the capacity of the model to generalize data is expressed in the lowering test loss values. As it can be later seen in Table \ref{tab:scal2}, there is not a direct correlation between training loss and testing loss.

Table \ref{tab:scal2} shows the result of a similar model in which both the model size and the internal embeddings' size change. Here, the number of epochs is set to $100$: this leads to better convergence on the training set but to worse results on the training set (with respect to Table \ref{tab:scal2}). It must also be kept in mind the fact that the series is quite short; hence, after a certain threshold, increasing the size of the network - and hence the number of parameters - won't affect results dramatically.
