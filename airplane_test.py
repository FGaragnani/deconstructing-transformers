...
def create_airline_datasets(sequence_length: int = 12, prediction_length: int = 4, normalization: Normalization = Normalization.MIN_MAX) -> Tuple[TensorDataset, np.ndarray]:
    """
...
def main():
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            if DELTA:
                prediction_series = reconstruct_from_deltas(predictions, original_series, SEQUENCE_LENGTH, PREDICTION_LENGTH)
            else:
                flat_preds = flatten_prediction_windows(predictions)
                prediction_series = np.full(len(original_series), np.nan)
    ...
    # model.save_model(path=f"checkpoints/airline_{test_loss:.6f}.pth")
